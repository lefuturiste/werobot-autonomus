let GyroHelper = require('./src/gyro.js')
let MouseHelper = require('./src/mouse.js')
let LightHelper = require('./src/light.js')
let ColorHelper = require('./src/color.js')
let OrientationHelper = require('./src/orientation.js')
let MoveYHelper = require('./src/moveY.js')
let Arduino = require('./src/arduino.js')

MouseHelper = new MouseHelper()
MouseHelper.init(false)

Arduino = new Arduino()

Arduino.init().then(() => {
    GyroHelper = new GyroHelper(Arduino)
    LightHelper = new LightHelper(Arduino)
    ColorHelper = new ColorHelper(Arduino)
    OrientationHelper = new OrientationHelper(Arduino, GyroHelper)
    MoveYHelper = new MoveYHelper(Arduino, MouseHelper, GyroHelper)
    Arduino.sendCommand('STOPALL').then(() => {
        Arduino.sendCommand('PING', [], true).then(response => {
            if (response.payload === 'pong!' && response.responseType === 'L') {
                console.log('GENERAL: ROBOT READY')
                ColorHelper.clear();
                LightHelper.untilDiscover().then(() => {
                    console.log('GENERAL: ROBOT DISCOVERED')
                    ColorHelper.setRgbColor(0, 10, 0);
                    MoveYHelper.goForwardY(60, 75).then(() => {
